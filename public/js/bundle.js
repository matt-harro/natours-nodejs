(() => {
  ...
  var timeout = (sec) => new Promise((_, reject) => {
    setTimeout(
  ...
  // public/js/utils/useFetch.js
  var REQUEST_TIMEOUT_SEC = 5;
  var useFetch = async (url, uploadData = null) => {
    try {
  ...
  async function updateSettings(data, type) {
    try {
      const query = type == "data" ? "/api/v1/users/updateMe" : "/api/v1/users/updateMyPassword";
  ...
  var bookTour = async (tourId) => {
    const res = await axios_default.get(`/api/v1/bookings/checkout-session/${tourId}`);
  ...
  var passwordFormEl = document.querySelector(".form-password");
